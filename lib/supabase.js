...
// In-memory promise-chain lock per name. Replaces navigator.locks, which is
// flaky on mobile Safari: concurrent getSession() calls occasionally throw
// "Lock was released because another request stole it", breaking queries.
// Serializing in-process is enough since we enforce a single client instance.
const memoryLocks = new Map()
const memoryLock = async (name, _acquireTimeout, fn) => {
  const prev = memoryLocks.get(name) || Promise.resolve()
