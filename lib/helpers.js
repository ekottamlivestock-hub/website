...
export function getNotificationColor(type) {
  if (type?.includes('approved') || type?.includes('delivered') || type?.includes('reactivated')) return 'text-emerald-600'
  if (type?.includes('rejected') || type?.includes('suspended') || type?.includes('removed')) return 'text-red-600'
 ...
 *
 * Goes through the server-side /api/notify-internal route, which:
 *   - authenticates the caller via cookie session
 *   - validates the (actor, recipient, type, metadata) tuple is a
 *     legitimate flow (admins skip the check)
 *   - inserts under service role, bypassing RLS but still tripping the
 *     rate-limit + actor-tracking triggers
 *
 * The browser can no longer INSERT into notifications directly — the
 * client RLS allows only self-inserts after migration 015. Cross-user
 * notifications must go through this helper.
 *
 * @param {string} userId   - Recipient user ID
 * @param {string} type     - Notification type (must be in the whitelist)
 * @param {string} message  - Notification message
 * @param {object} metadata - JSON metadata, including foreign keys the
 *                            server uses to verify the relationship
 *                            (e.g. order_id, listing_id, user_id).
 */
export async function sendNotification(userId, type, message, metadata = {}) {
  try {
    const res = await fetch('/api/notify-internal', {
      method: 'POST',
    ...
    if (!res.ok) {
      ...
      console.error('Notification send error:', res.status, detail.error)
    }
  } catch (err) {
